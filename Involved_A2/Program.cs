...
var sampleGetResponse = await client.GetFromJsonAsync<Dictionary<String, int>>(sampleUrl);
...
List<int> GetAnswer(Dictionary<String, int> verdiepen)
{
    int gewensteVerdiep = verdiepen["destination"];
    List<int> res = new List<int>();
    res.Add(0);
    for(int i = 1; i <= gewensteVerdiep; i++)
    {
        res.Add(-i);
        res.Add(i);
    }
